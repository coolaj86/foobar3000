...
(function () {
  ...
  function onClientCreatedSocket() {
    console.log("[Response] On Created Socket");
  }
  ...
  function onMessage(message, rinfo) {
    var client = dgram.createSocket("udp4", onClientCreatedSocket);
      ;

    console.log("[Listener] On Message: " + message + " from " + rinfo.address + ":" + rinfo.port);
    client.send(message, 0, message.length, rinfo.port, rinfo.address, onSent);
  }
